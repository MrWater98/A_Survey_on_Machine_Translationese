...
\begin{tikzpicture}[
    ...
    milestone/.style={circle, fill=blue!70, text=white, minimum size=0.6cm, font=\tiny\bfseries},
    era/.style={rectangle, rounded corners, fill=gray!20, minimum height=1.2cm, font=\footnotesize\bfseries},
    feature/.style={rectangle, rounded corners, fill=orange!20, text width=3.2cm, align=center, font=\tiny},
    method/.style={rectangle, rounded corners, fill=green!20, text width=3.2cm, align=center, font=\tiny}
]

% Timeline base
\draw[line width=2pt, gray!50] (0,0) -- (16,0);

% Era backgrounds
\fill[blue!5] (0,-0.5) rectangle (4,3.5);
\fill[orange!5] (4,-0.5) rectangle (8,3.5);
\fill[green!5] (8,-0.5) rectangle (12,3.5);
\fill[red!5] (12,-0.5) rectangle (16,3.5);

% Era labels at top
\node[era, fill=blue!10] at (2,3.2) {Corpus-Based};
\node[era, fill=orange!10] at (6,3.2) {SMT Era};
\node[era, fill=green!10] at (10,3.2) {NMT Era};
\node[era, fill=red!10] at (14,3.2) {LLM Era};

% Time markers on timeline
\node[below] at (0,0) {\tiny 1990s};
\node[below] at (4,0) {\tiny 2003};
\node[below] at (8,0) {\tiny 2015};
\node[below] at (12,0) {\tiny 2020};
\node[below] at (16,0) {\tiny 2025};

% Layer 1: Key Milestones (top layer, y=2.5)
\node[milestone] (baker) at (1,2.5) {};
\node[above=0.1cm of baker, text width=2.5cm, align=center, font=\tiny] {1993: Baker\\Translation Universals};

\node[milestone] (koehn) at (5,2.5) {};
\node[above=0.1cm of koehn, text width=2.5cm, align=center, font=\tiny] {2003: Koehn et al.\\Phrase-based SMT};

\node[milestone] (bahdanau) at (8.5,2.5) {};
\node[above=0.1cm of bahdanau, text width=2.5cm, align=center, font=\tiny] {2015: Bahdanau\\Attention Mechanism};

\node[milestone] (vaswani) at (10.5,2.5) {};
\node[above=0.1cm of vaswani, text width=2.5cm, align=center, font=\tiny] {2017: Vaswani\\Transformer};

\node[milestone] (llm) at (13,2.5) {};
\node[above=0.1cm of llm, text width=2.5cm, align=center, font=\tiny] {2020+: GPT\\LLM Translation};

% Connecting arrows for milestones
\draw[->, thick, blue!50] (baker) -- (koehn);
\draw[->, thick, blue!50] (koehn) -- (bahdanau);
\draw[->, thick, blue!50] (bahdanau) -- (vaswani);
\draw[->, thick, blue!50] (vaswani) -- (llm);

% Layer 2: Translationese Characteristics (middle layer, y=1.2)
\node[feature] at (2,1.2) {
    \textbf{Translation Universals}\\
    • Simplification\\
    • Normalization\\
    • Explicitation
};

\node[feature] at (6,1.2) {
    \textbf{Awkward Literalism}\\
    • Source interference\\
    • Word-for-word idioms\\
    • Unnatural word order
};

\node[feature] at (10,1.2) {
    \textbf{Deceptive Fluency}\\
    • Lexical diversity loss\\
    • Over-simplification\\
    • Post-editese
};

\node[feature] at (14,1.2) {
    \textbf{Subtle Over-Adherence}\\
    • Excessive literalism\\
    • Over-normalization\\
    • Stylistic flattening
};

% Layer 3: Detection Methods (bottom layer, y=-0.3)
\node[method] at (2,-0.3) {
...
\node[method] at (10,-0.3) {
    \textbf{Neural Approaches}\\
    BERT, LSTM\\
    Feature learning
};

\node[method] at (14,-0.3) {
    \textbf{LLM-Based}\\
    Prompting, Few-shot\\
    Zero-shot detection
};

% Warning indicator for model collapse
\node[draw=red, thick, rectangle, rounded corners, fill=red!10, text width=2.5cm, align=center, font=\tiny] at (15.5,-1.5) {
    \textbf{⚠ Risk:}\\
    Model Collapse\\
    Data Contamination
};
...
% Legend
\node[anchor=west, font=\tiny] at (0,-2.5) {\textbf{Legend:}};
\node[milestone, minimum size=0.4cm] at (1.5,-2.5) {};
\node[anchor=west, font=\tiny] at (1.8,-2.5) {Key Milestone};
\node[feature, text width=2cm, minimum height=0.4cm] at (4.5,-2.5) {Translationese};
\node[method, text width=2cm, minimum height=0.4cm] at (7,-2.5) {Detection Method};
