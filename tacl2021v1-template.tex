...
\maketitle
\begin{abstract}
  The field of machine translation (MT) has undergone rapid evolution, with techniques advancing from statistical (SMT) and neural (NMT) paradigms to the recent dominance of Large Language Models (LLMs). A persistent byproduct of this technological progression is ``translationese''---systematic linguistic artifacts that distinguish machine-translated text from human-written text. The characteristics of translationese have co-evolved with MT systems, shifting from overt grammatical errors in early models to more subtle biases such as simplification, normalization, and excessive literalism in modern NMT and LLM outputs. In response, a significant body of research has emerged to monitor, document, and mitigate these effects. This paper surveys this research landscape, presenting a chronological overview of how the study of translationese has developed. We review the progression of methods for its detection, from feature-engineering to deep learning classifiers; the techniques for its characterization across different MT architectures; and the strategies developed for its mitigation, including data filtering, model fine-tuning, and prompt engineering. Finally, we highlight ongoing challenges and future directions in managing the pervasive influence of translationese in an era of ubiquitous MT.
\end{abstract}
